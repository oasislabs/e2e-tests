...
library ModexpPrecompile {
  function modexp(bytes memory base, bytes memory exponent, bytes memory modulus) internal returns (bool success, bytes memory output) {
...
library BytesTool {
  function memcopy(bytes memory src, uint srcoffset, bytes memory dst, uint dstoffset, uint len) pure internal {
    assembly {
