...
contract CallableCounter {
  ...
  constructor(address peer, uint256 count, bool isTail) public {
  ...
  function delegatecallIncrementCounter() public {
    ...
    /// Recursively calls peer until the tail counter is incremented via delegatecall.
  function incrementTailCounterDelegatecall() public {
	  _sender = msg.sender;
	  _peer.incrementPeerCounterDelegatecall();
  }

  function incrementPeerCounterDelegatecall() public {
	if (_peer.isTail()) {
	  _sender = msg.sender;
	  bool status = address(_peer).delegatecall(bytes4(keccak256("receiveDelegatecallIncrementCounter()")));
	  emit Status(status);
	  ...
	  _sender = msg.sender;
	  incrementPeerCounterDelegatecall();
	}
  }
